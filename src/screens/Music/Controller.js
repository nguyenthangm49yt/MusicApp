...
export default function Controller({onNext, onPrv}) {
  ...
  const returnPlayBtn = () => {
    switch (isPlaying.current) {
      case 'playing':
        return <Icon color="#000" name="pause" size={45} />;
      case 'paused':
        return <Icon color="#000" name="play-arrow" size={45} />;
      default:
  ...
  return (
    <View style={styles.container}>
      ...
      <Icon color="#000" name="skip-previous" size={45} />
      </TouchableOpacity>
      <TouchableOpacity onPress={onPlayPause}>
        {returnPlayBtn()}
      </TouchableOpacity>
      <TouchableOpacity onPress={onNext}>
        <Icon color="#000" name="skip-next" size={45} />
      </TouchableOpacity>
    </View>
  );
}
